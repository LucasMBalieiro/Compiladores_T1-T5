...
public class AnalisadorSemantico extends GrammarT3BaseVisitor<Void> {

    Escopos escopos = new Escopos();

    @Override
    public Void visitPrograma(GrammarT3Parser.ProgramaContext ctx) {
        return super.visitPrograma(ctx);
    }

    @Override
    public Void visitDeclaracaoGlobal(GrammarT3Parser.DeclaracaoGlobalContext ctx) {
    ...
    public Void visitDeclaracaoLocal(GrammarT3Parser.DeclaracaoLocalContext ctx) {
        ...
        if (ctx.tipo() != null) { // É uma declaração de tipo customizado
            ...
            for (GrammarT3Parser.IdentificadorContext identCtx : ctx.variavel().identificador()) {
    ...
    private TabelaDeSimbolos criarTabelaParaRegistro(GrammarT3Parser.RegistroContext ctx) {
        TabelaDeSimbolos tabelaCampos = new TabelaDeSimbolos();

        for (GrammarT3Parser.VariavelContext varCtx : ctx.variavel()) {
            String tipoStr = varCtx.tipo().getText();
            TabelaDeSimbolos.TipoLA tipoCampo = determinarTipoDeString(tipoStr);

            for (GrammarT3Parser.IdentificadorContext identCtx : varCtx.identificador()) {
